...
const CreateBook = () => {
  ...
  const [condition, setConditional] = useState('BRANDNEW')
  const [, createBook] = useMutation(CreateBookMutation)
  const router = useRouter();
  const submit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const variables = {title: title, description: desc, image: img, condition: condition};
  ...
  return (
    <>
      <form className="border-2 max-w-min p-5 flex flex-col mx-auto mt-32" onSubmit={submit}>
        ...
        <select value={condition} className="border-2 my-2" onChange={event => setConditional(event.target.value)}>
