...
const BookWrapper = ({book, size, className}: {book: Book, size?: string, className?: string}) => {
  ...
  return (
    ...
    >
      <Link href={`/book/${id}`}>
        <a style={{width: isSmall ? 200 : 224}} className={`block max-w-full w-full h-full ${isSmall ? 'p-4' : ''}`}>
